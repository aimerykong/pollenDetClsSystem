...
opts.expDir = fullfile('./exp', 'main041_cls_res50_alldata_avgPool_preSegMasking');
...
bopts.imdb = imdb;
%% train
fn = getImgBatchWrapper_pollen4cls_predSegMasking(bopts);
...
existList = zeros(1, length(imdb.labelList));
for i = 1:length(existList)
    if exist(imdb.imgList{i}, 'file') && exist(imdb.maskList{i}, 'file') 
        existList(i) = 1;
    elseif ~exist(imdb.imgList{i}, 'file') && exist(imdb.maskList{i}, 'file')
        existList(i) = -2;
    elseif exist(imdb.imgList{i}, 'file') && ~exist(imdb.maskList{i}, 'file')
        existList(i) = -1;
    else
