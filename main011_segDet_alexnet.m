...
netbasemodel = load(fullfile(path_to_model, 'imagenet-matconvnet-alex.mat' ));

netbasemodel.layers(1).inputs = {'data'};
netbasemodel.vars(1).name = 'data';
...
netbasemodel.removeLayer('fc8'); % remove layer
netbasemodel.removeLayer('relu7'); % remove layer
netbasemodel.removeLayer('fc7'); % remove layer
netbasemodel.removeLayer('relu6'); % remove layer
netbasemodel.removeLayer('fc6'); % remove layer
netbasemodel.removeLayer('pool5'); % remove layer


netbasemodel.layers(1).block.pad = [5 5 5 5];
netbasemodel.layers(1).block.stride = [2 2];


% move pool3
% netbasemodel.layers(6).block.stride = [1, 1];
netbasemodel.layers(6).block.pad = [1, 1, 1, 1];


sName = 'x17';
lName = 'res6_conv';
block = dagnn.Conv('size', [1 1 256 2], 'hasBias', true, 'stride', 1, 'pad', 0, 'dilate', 1);
...
weights = randn(1, 1, 256, 2, 'single')*sqrt(2/5);
...
opts.expDir = fullfile('./exp', 'main011_segDet_alexnet');
