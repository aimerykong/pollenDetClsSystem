    ...
    for epoch in range(num_epochs):        
        ...
        for phase in ['train', 'test']:
            ...
            for sample in dataloaders[phase]:                
                image, grndSeg, mask_distanceTransform, mask_overlap, mask_voteX, mask_voteY, mask_peaks, mask_radius = sample
                
                image = image.to(device)
                grndSeg = grndSeg.to(device) 
                mask_voteX = mask_voteX.to(device)
                mask_voteY = mask_voteY.to(device)
                ...
                with torch.set_grad_enabled(phase=='train'):
                    ...
                    for ii in range(len(scaleList)):
                        ...
                        grndYXOffset = torch.cat((mask_voteY, mask_voteX), 1)
                        
                        loss_seg = 0
                        loss_reg = 0
                        loss_seg = lossFunc_seg(predSeg, grndSeg)
                        
                        grndSeg = grndSeg.repeat(1, 2, 1, 1)
                        mask_overlap = mask_overlap.repeat(1, 2, 1, 1)
                        #print('grndYXOffset ', grndYXOffset.shape)
                        #print('predYXOffset ', predYXOffset.shape)
                        #print('grndSeg ', grndSeg.shape)
                        #print('mask_overlap ', mask_overlap.shape)
                        
                        loss_reg = lossFunc_reg(grndYXOffset, predYXOffset, grndSeg, mask_overlap)                        
